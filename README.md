...
### Two outputs, changed two different ways

`vss-codex` emits two things, and they are modified differently — worth understanding before you tweak
anything:

- **The knowledge base** (`out/reference/` — `api/*.md`, `events.md`, `enums.md`, `lib/`, `harmony/`,
  and the `decompiled/` tree) is **generated by the tool** from the game binaries. Its *content* doesn't
  exist as static files — it's re-emitted on every run by the C# in `src/VssCodex/`. So changing what the
  base contains means changing the **generator**, which requires **re-running the tool** (a full build
  re-decompiles and regenerates the docs).
- **The skill** (`skill/SKILL.md.template`, `skill/references/`, `skill/examples/`) is **hand-written
  content**. The renderer only substitutes placeholders (`{{REFERENCE_PATH}}`, `{{VS_VERSION}}`, …) and
  copies `references/` + `examples/` **verbatim**. So to refine the skill's *behaviour* — its routing,
  its rules, how it cites sources, the teaching docs and examples — you just **edit those markdown
  files**. No decompiling and no base rebuild is involved; only a re-render, which is the copy step every
  run performs anyway.

### Refining the skill, and how a change reaches users

The skill is plain markdown you can tune freely under `skill/`. A change there propagates like this:

1. **Edit** `skill/SKILL.md.template`, `skill/references/*`, or `skill/examples/*`, and commit it.
2. Anyone who **clones/downloads the repo** gets the updated template right away. They run `vss-codex`
   (which they do anyway, to build the base against their own game install); the render fills the
   placeholders and copies your edited references/examples into `out/.claude/skills/vss/`. Their skill now
   carries your behaviour change — no extra step, because skill edits are just copied, not regenerated.
3. Users of the **prebuilt release binaries** receive it only in a **new release**: each binary bundles
   the skill source at build time, so cut a new tag (CI rebuilds the binaries) to ship the change to them.

In short — **edit the skill source → commit/push → repo users get it on their next run, binary users on
the next release.** The rendered `out/.claude/skills/vss/` is a build artifact: don't hand-edit it; edit
the source in `skill/` so the change is reproducible, tested, and shipped to everyone.
